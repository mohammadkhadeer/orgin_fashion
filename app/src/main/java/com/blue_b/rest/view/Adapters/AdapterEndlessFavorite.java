...
public class AdapterEndlessFavorite extends RecyclerView.Adapter<BaseViewHolderUser>{
      ...
      public void onClick(View v) {
        ...
        {}else{
          if (getItem(position).isInStock()){
            ItemTest itemTest = Functions.itemFav(getItem(position));
  ...
  private void fillText(TextView adapter_fav_name_tv, TextView adapter_fav_store_name, TextView adapter_fav_per, int position, Context context) {
    adapter_fav_name_tv.setText(Functions.getTextEngOrLocal(context,getObject(position).getName(),getObject(position).getName_local()));
    //change ite to arabic in store object
    adapter_fav_store_name.setText(Functions.getTextEngOrLocal(context,getObject(position).getStore().getName(),getObject(position).getStore().getName()));
    adapter_fav_per.setText(Functions.calculatePercentage(getObject(position).getPrice(),getObject(position).getDiscountPrice()));
  }
