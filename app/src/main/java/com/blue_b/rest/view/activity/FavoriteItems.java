...
import com.blue_b.rest.view.Adapters.AdapterEndlessFavorite;
import com.blue_b.rest.functions.FillItem;
...
import static com.blue_b.rest.functions.Functions.itemFav;
...
public class FavoriteItems extends AppCompatActivity {
    ...
    private int currentPage = FragmentResults.PAGE_START;
...
//        dbHelper.deleteAllFavItems();
    ...
    private void fillItemIdPag() {
        newItemsMustLoaded = Functions.getNewItems(itemsId,newItemsMustLoaded);
    ...
    private void checkIfNeedLoadMore() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (currentPage != FragmentResults.PAGE_START ) adapterEndlessFavorite.removeLoading();
                if (itemFavoriteLoadMoreArrayList.size()!=0)
                {
                    adapterEndlessFavorite.addItems(itemFavoriteLoadMoreArrayList);
                }

                if (Functions.loadMoreItem(itemsId.size(),itemFavoriteArrayList.size()) != 0)
                {
    ...
    private void intiRetrofit(String itemID) {
        retrofit = RetrofitFunctions.getSingleItemDetails(itemID);
        ...
        callHome.enqueue(new Callback<ItemFavorite>() {
            ...
            public void onResponse(Call<ItemFavorite> call, Response<ItemFavorite> response) {
                if (!response.isSuccessful())
                {
                    ...
                    ItemFavorite itemFavorite = Functions.itemFav();
    ...
    private void getItemsIdFromDtaBase() {
        itemsId = FillItem.itemsIdInFavorite(this);
    }
