...
//        followingTV.setText(String.valueOf(followingArrayList.size()));
        if (LoginInfo.getLoginOrNotFromSP(getActivity()).isEmpty())
        {
    ...
    private void fillUserInfo() {
        Picasso.get()
                .load(LoginInfo.getPhotoFromSP(getActivity()))
                .fit()
                .centerCrop()
                .into(userImageIV);

        profile_details_user_name_TV.setText(LoginInfo.getNameFromSP(getActivity()));
        profile_details_user_email.setText(LoginInfo.getEmailFromSP(getActivity()));
        if (LoginInfo.getTypeFromSP(getActivity()).equals("google"))
        {
            Picasso.get()
                    .load(R.drawable.g)
                    .fit()
