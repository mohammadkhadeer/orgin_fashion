...
//    List<Offer> mList = new ArrayList<>();
    ...
    public void onBindViewHolder(final AdapterType2.ViewHolder holder, final int position) {
        ...
        Functions.checkFavOrNot(dealsArrayL.get(position).getId(),context,holder.fav_imageView);
    ...
    private void fillText(ViewHolder holder, Context context, int position) {
        ...
        holder.per.setText(Functions.calculatePercentage(dealsArrayL.get(position).getPrice(),dealsArrayL.get(position).getDiscountPrice()));
    }
