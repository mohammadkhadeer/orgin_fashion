...
import com.blue_b.rest.view.categoriesComp.FillSlider;
import com.blue_b.rest.view.categoriesComp.FillType2;
import com.blue_b.rest.view.categoriesComp.FillType4;
...
//      oldPrice = (TextView) itemView.findViewById(R.id.adapter_old_price_TV);
    ...
    public void onBind(int position) {
      ...
      if (getObject(position).getSub_cat().getAppearance().equals("1"))
      {
        container_slider_rl.setVisibility(View.GONE);
            FillType2.fillCaseItem(recyclerViewT2,context,type4_cat_name_TV,see_all_type4_rl,homeItemsList.get(position),lodaingRVT2,adapter_see_all_tv);
      }

      if (getObject(position).getSub_cat().getAppearance().equals("2"))
      {
        container_slider_rl.setVisibility(View.GONE);
        FillType4.fillCase4Item(recyclerViewT4,context,type4_cat_name_TV,see_all_type4_rl,homeItemsList.get(position),lodaingRVT4,adapter_see_all_tv);
      }

      if (getObject(position).getSub_cat().getAppearance().equals("3"))
      {
        container_slider_rl.setVisibility(View.VISIBLE);
        FillSlider.fillSliderItemsItem(sliderView,context,type4_cat_name_TV,see_all_type4_rl,homeItemsList.get(position),adapter_see_all_tv);
      }
  ...
  private void fillText(TextView nameTV, int position, Context context) {
    nameTV.setText(Functions.getTextEngOrLocal(context,getObject(position).getSub_cat().getName_en(),getObject(position).getSub_cat().getName_local()));
