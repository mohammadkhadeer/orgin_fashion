package com.blue_b.rest.view.Adapters.resultTupe1;
...
public class FillResultsType1 {
    ...
    private static void fillOffer(RelativeLayout offerCover, TextView offerPer,ItemTest deal) {
        ...
        offerPer.setText(Functions.calculatePercentage(deal.getPrice(),deal.getDiscountPrice()));
    }

    private static void fillTV(ItemTest itemTest, TextView name,Context context) {
        name.setText(Functions.getTextEngOrLocal(context,itemTest.getName(),itemTest.getName_local()));
    }
    ...
    private static void fillImage(Context context, ImageView imageView, ItemTest itemTest) {
        ...
        Picasso.get()
                .load(API.apiURLBase()+itemTest.getFlagArrayL().get(0).getUrl())
                .fit()
