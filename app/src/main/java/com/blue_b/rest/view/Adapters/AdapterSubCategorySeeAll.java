...
//    List<Offer> mList = new ArrayList<>();
    ...
    private void fillImage(Context context, ViewHolder holder, int position) {
        Picasso.get()
                .load(API.apiURLBase()+"/"+sub_catArrayList.get(position).getFlag().getUrl())
                .fit()
                .centerCrop()
    ...
    private void fillText(ViewHolder holder, Context context, int position) {
        holder.cityOrAreaTV.setText(Functions.getTextEngOrLocal(context,sub_catArrayList.get(position).getName_en(),sub_catArrayList.get(position).getName_local()));
    }
